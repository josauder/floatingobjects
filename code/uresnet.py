        ...
        if name:
            try:
                ...
                if isinstance(sub_layer, torch.nn.BatchNorm2d):
                    ...
                    layer._modules[name] = torch.nn.GroupNorm(num_groups=2, num_channels=num_channels, eps=10e-5, affine=True)
                elif isinstance(sub_layer, torch.nn.BatchNorm1d):
                    num_channels = sub_layer.num_features
                    print("replacing", name)
                    # first level of current layer or model contains a batch norm --> replacing.
                    layer._modules[name] = torch.nn.GroupNorm(num_groups=2, num_channels=num_channels, eps=10e-5, affine=True)
...
class SegmentationEncoder(torch.nn.Module):
    ...
    def forward(self, x1):
        ...
        """
        feats = [x1]#self.concatenate(x1, x2)]
